...
func NewManager(cfg links.SecureLinkConfigurator) (*Manager, error) {
	if cfg == nil {
...
func (m *Manager) Generate(route string, payloads ...links.SecureLinkPayload) (string, error) {
...
func (m *Manager) GetAndValidate(fn func(string) string) (links.SecureLinkPayload, error) {
	if m == nil || m.inner == nil {
		return nil, errors.New("securelink manager not configured")
	}
	...
	return links.SecureLinkPayload(payload), nil
}
...
func toPayloads(payloads []links.SecureLinkPayload) []urlkit.Payload {
	if len(payloads) == 0 {
		return nil
	}
