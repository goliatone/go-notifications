...
| `notification_events` | Incoming events awaiting fan-out | `definition_code`, `tenant_id`, `actor_id`, `recipients` (JSON array), `context` (JSON), correlation/request IDs, scoped idempotency identity and fingerprint, transient marker, publication/digest references, retry lease, `scheduled_at`, `status` |
| `notification_messages` | Expanded, rendered or redacted message state | `event_id` (FK), `retry_operation_id`, `channel`, `locale`, content/URL projection, opaque `receiver`, `status`, safe `metadata` (JSON) |
...
`002`, and mandatory validation targets `sqlite` and `postgres`. A host that
needs graph dependencies can pass source keys, for example
...
The PostgreSQL upgrade migration can remove its added columns on downgrade.
SQLite's upgrade down migration removes added tables and indexes but retains
columns because portable column removal would require table reconstruction.
Hosts should restore from backup when a complete SQLite schema rollback is
required.
